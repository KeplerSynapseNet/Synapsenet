...
void PoeV1Engine::setStaticValidators(const std::vector<crypto::PublicKey>& validators) {
    std::lock_guard<std::mutex> lock(impl_->mtx);
    impl_->validators = validators;
    std::sort(impl_->validators.begin(), impl_->validators.end(), pubKeyLess);
    impl_->validators.erase(std::unique(impl_->validators.begin(), impl_->validators.end()), impl_->validators.end());
}
...
PoeSubmitResult PoeV1Engine::submit(
    ...
    {
        std::lock_guard<std::mutex> lock(impl_->mtx);
        std::string ckey = "poe:v1:contentid:" + hex32(cidPre);
    ...
    for (;;) {
        ...
        if (poe_v1::hasLeadingZeroBits(entry.submitId(), entry.powBits)) break;
    }
