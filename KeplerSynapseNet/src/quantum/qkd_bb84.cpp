...
std::vector<uint8_t> QKDBB84::privacyAmplification(const std::vector<uint8_t>& siftedKey,
    ...
    if (siftedKey.empty()) {
        return std::vector<uint8_t>();  // Return empty key, caller should check
    }
    
    // Apply privacy amplification using hashing
    // Output length scales with error rate: longer keys needed if error rate is high
    size_t outputLen = static_cast<size_t>(siftedKey.size() * (1.0 - 2.0 * errorRate));
    if (outputLen < 32) outputLen = 32;
    
    // Use SHA-256 for privacy amplification
    auto hash = crypto::sha256(siftedKey.data(), siftedKey.size());
    
    // Return hash as base for amplified key (caller can extend with additional rounds if needed)
