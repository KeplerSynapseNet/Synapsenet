...
std::string TorFetch::fetch(const std::string& url) {
    ...
    
    {
        ...
        cfg = impl_->config;
        tor = impl_->external ? impl_->external : impl_->owned.get();
        impl_->stats.requests++;
    }
    
    std::string response;
    std::string socks;
    if (!cfg.socksHost.empty() && cfg.socksPort != 0 && tor && tor->isConnected()) {
    ...
    response = res.body;
    if (res.exitCode != 0 && onion && parsed.scheme == "https") {
